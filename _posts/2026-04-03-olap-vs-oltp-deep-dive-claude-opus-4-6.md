---
layout: fermented
title: 1행을 빨리 찾을 것인가, 10억 행을 빨리 훑을 것인가 — OLTP vs OLAP 아키텍처 해부
date: 2026-04-03
categories:
  - development
tags:
...
feed: false
sitemap: false
description: 스토리지 엔진부터 쿼리 실행 모델, HTAP까지 — OLTP와 OLAP가 왜 근본적으로 다른 설계를 가질 수밖에 없는지 내부 구조를 해부한다
---

## 왜 이 구분이 중요한가

데이터베이스를 다루는 엔지니어라면 OLTP(Online Transaction Processing)와 OLAP(Online Analytical Processing)라는 용어를 수없이 들어왔을 것이다. "하나는 트랜잭션용, 하나는 분석용" — 교과서적 설명은 여기서 끝난다.

하지만 이 두 패러다임이 **왜 근본적으로 다른 설계 철학을 가질 수밖에 없는지**, 그 내부 구조를 이해하면 기술 선택의 근거가 달라진다. 스토리지 엔진에서 동시성 제어, 쿼리 실행 모델, 그리고 HTAP(Hybrid Transactional/Analytical Processing)까지 — 이 글에서 하나씩 해부한다.

---

## 1. 근본적 설계 목표의 차이

OLTP와 OLAP의 차이는 기능 차이가 아니다. **최적화 대상 자체가 다르다.**

**OLTP**는 개별 레코드 수준의 읽기/쓰기 지연 시간(Latency)을 최소화한다. 한 번의 요청에서 접근하는 행(row)은 보통 1~수십 개에 불과하지만, 초당 수천에서 수만 건의 트랜잭션을 동시에 처리해야 한다. 핵심 지표는 **TPS(Transactions Per Second)**와 **p99 latency**이다.

**OLAP**는 대량 데이터에 대한 집계(Aggregation) 쿼리의 처리량(Throughput)을 극대화한다. 한 번의 쿼리에서 수백만~수십억 행을 스캔하지만, 동시 쿼리 수는 상대적으로 적다. 핵심 지표는 **QPS(Queries Per Second)**와 **Scan Throughput (rows/sec)**이다.

이 근본적인 차이가 이하의 모든 아키텍처적 분기점을 만든다.
...
OLTP 시스템의 대표적인 스토리지 방식이다. PostgreSQL, MySQL(InnoDB), Oracle 등이 사용한다.
...
한 행의 모든 컬럼이 물리적으로 인접해 저장된다. 이 구조의 이점은 명확하다:

- **Point Lookup이 빠르다**: `SELECT * FROM users WHERE id = 1`은 인덱스를 통해 단일 디스크 페이지만 읽으면 된다. 모든 컬럼이 같은 페이지에 있으므로 I/O가 최소화된다.
- **단일 행 INSERT/UPDATE가 효율적이다**: 새 레코드를 한 페이지의 빈 슬롯에 기록하면 끝이다. 컬럼별로 다른 위치에 쓸 필요가 없다.

그러나 **분석 쿼리에서는 치명적인 비효율**이 발생한다. `SELECT AVG(age) FROM users`를 실행하면, `age` 컬럼만 필요한데도 `name`, `city` 등 불필요한 컬럼까지 디스크에서 읽어야 한다. 100개 컬럼이 있는 테이블에서 2개만 필요하면, 이론적으로 I/O의 98%가 낭비된다.

### 2.2 Column-Oriented Storage (열 기반 저장)

OLAP 시스템의 핵심 저장 방식이다. ClickHouse, Apache Parquet, DuckDB, Amazon Redshift 등이 사용한다.
...
같은 컬럼의 값들이 물리적으로 연속 저장된다. 분석 쿼리에 압도적인 이유는 세 가지다:

**첫째, I/O 선택성(I/O Selectivity)** 이 극대화된다. `SELECT SUM(revenue) FROM orders`를 실행하면, `revenue` 컬럼 데이터만 디스크에서 읽는다. 100개 컬럼 중 1개만 읽으면 이론적으로 I/O가 100분의 1로 줄어든다. 실제로는 메타데이터 오버헤드 등으로 **수십 배 수준**의 I/O 감소를 달성한다.

**둘째, 압축률(Compression Ratio)** 이 비약적으로 향상된다. 같은 타입의 데이터가 연속 저장되면, 데이터 간 유사성이 높아져 압축이 극히 효율적이 된다:
- `city` 컬럼에 "Seoul"이 반복되면 **Dictionary Encoding**으로 정수 인덱스만 저장
- `age` 컬럼처럼 범위가 좁은 정수는 **Frame-of-Reference (FOR)** 인코딩으로 차이값만 저장
- **Run-Length Encoding (RLE)**으로 연속된 동일 값을 압축

데이터 특성에 따라 차이가 크지만, 컬럼 스토어는 행 스토어 대비 **3~10배** 이상의 압축률을 달성하는 것이 일반적이다.

**셋째, CPU 벡터화(Vectorized Execution)**와의 시너지이다. 컬럼 데이터가 메모리에 연속 배치되면 CPU의 SIMD(Single Instruction, Multiple Data) 명령어를 활용해 한 번의 CPU 사이클로 여러 값을 동시에 처리할 수 있다. SIMD 자체로는 4~16배 수준의 처리량 향상을 제공하지만, 캐시 효율 개선과 함수 호출 오버헤드 감소까지 합치면 집계 연산에서 **수십 배**의 성능 차이로 이어진다.

### 2.3 Write Amplification 트레이드오프

컬럼 스토어의 약점은 **쓰기(Write) 성능**이다. 하나의 행을 삽입하려면 각 컬럼 파일에 개별적으로 값을 기록해야 한다. 100개 컬럼이 있다면 100번의 쓰기가 발생한다. 이것이 OLTP 워크로드에서 컬럼 스토어가 비효율적인 근본 이유이며, ClickHouse 같은 시스템이 **LSM-Tree 기반 MergeTree**를 사용해 작은 배치를 버퍼링한 뒤 백그라운드에서 병합하는 이유이기도 하다.
...
OLTP 시스템의 핵심 인덱스 구조는 **B-Tree(Balanced Tree)**이다:
...
PostgreSQL은 B-Tree 외에도 GiST, GIN, BRIN 등 다양한 인덱스를 지원하지만, 대부분의 OLTP 워크로드에서 B-Tree가 기본이다.

### 3.2 OLAP: Zone Map, Sparse Index, 그리고 Data Skipping

OLAP 시스템은 전통적인 B-Tree 대신 **대량 스캔에 최적화된 인덱스**를 사용한다.

**Zone Map (Min/Max Index)**: 각 데이터 블록(보통 수천~수만 행)의 최솟값과 최댓값을 메타데이터로 저장한다. `WHERE date > '2025-01-01'` 쿼리가 들어오면, 최댓값이 2024-12-31인 블록은 아예 읽지 않고 건너뛴다(Data Skipping). ClickHouse에서는 이를 **Primary Key Index**라 부르며, 8,192행 단위의 Granule마다 min/max를 기록한다.

**Sparse Index**: 모든 행이 아닌, 일정 간격(예: 8,192행마다)의 키 값만 인덱스에 저장한다. 메모리 사용량이 극히 적어 수십억 행의 테이블도 인덱스가 메모리에 완전히 올라간다.
